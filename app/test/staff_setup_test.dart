...
void main() {
  testWidgets('define a senha e confirma o sucesso', (tester) async {
    ...
    final mock = MockClient((req) async {
      ...
      return http.Response(
          jsonEncode({'status': 'password_set', 'mfa_enabled': false}), 200);
    });
  ...
  testWidgets('link inválido mostra o erro do servidor e mantém o formulário', (tester) async {
    final mock = MockClient((_) async => http.Response(
        jsonEncode({'title': 'Link inválido',
  ...
  testWidgets('falha de conexão não deixa a tela travada carregando', (tester) async {
    final mock = MockClient((_) async => throw const SocketExceptionFake());
  ...
  testWidgets('a tela também existe em inglês', (tester) async {
    ...
    await tester.pumpWidget(_app(_api(mock), locale: const Locale('en')));
...
/// Exceção de rede qualquer (não vale importar dart:io num teste que roda na web).
