...
| R-04 | Sham ativo (Δf=0) com **UI idêntica**; visualização **não reativa** ao áudio; handle neutro ao cliente; headers de áudio neutros; 429 do endpoint público idêntico nos dois braços; guard de produção da chave selada; **o áudio guardado no aparelho fica cifrado** (chave no Keystore/Keychain), porque um arquivo em claro revelaria o braço por FFT | Inegociáveis #1/#2; ADR-053/077/082/090/**103** |
| R-05 | Aviso persistente "ferramenta complementar, não substitui cuidado profissional"; postura anti-*overclaim* obrigatória em toda copy; tela de EA grave reforça **192/CVV 188**; **rastreio ativo de risco** (PHQ-9 item 9 / GAD-7 >= 15) que **retira do protocolo**, abre ficha de encaminhamento e orienta CVV/SAMU — sem devolver escore ao participante | `CLAUDE.md` (postura científica); ADR-073; **ADR-102** |
