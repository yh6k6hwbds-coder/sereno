...
| 113 | **A lista de participantes deixa de responder errado em silêncio:** `GET /research/participants` era um stub que devolvia **200 com `items: []`** e um `TODO` no corpo — pior que rota faltando, porque a ausente dá 404 e esta fazia concluir que **não há participantes**. Implementada como o contrato já prometia (braço **codificado**, nulo = ainda não randomizado; adesão, sessões e eventos; keyset em `enrolled_at, id`, que desempata inscrições no mesmo instante). As contagens saem por **subconsulta**: um `join` de sessões com eventos adversos daria o **produto** das duas e a adesão sairia multiplicada por um fator — sem gritar, porque o número continua plausível **[novo]** |

| 114 | **Distribuição do app, e a sessão só onde a fidelidade vale:** **Android por link direto**, com APK **assinado com a chave do estudo** — hoje o CI o assinava com a chave de **debug**, que é pública, que loja nenhuma aceita e cuja troca obriga a desinstalar (levando o cache cifrado junto). Sem keystore o artefato passa a sair nomeado `sereno-apk-DEBUG-NAO-DISTRIBUIR`, e segredo pela metade **falha alto**. A pergunta aberta da fidelidade virou **trava**: a sessão **não abre no navegador**, onde o áudio é reamostrado pela pilha do browser e o caminho nunca foi validado — deixar rodar seria coletar dado sobre um estímulo que o estudo não consegue afirmar qual é. O bloqueio é da SESSÃO (diário, questionários e a tela de senha da equipe seguem na web), e `isWeb` é **injetável** porque `kIsWeb` é `const`: trava que nenhum teste alcança some na primeira refatoração **[novo]** |

| 115 | **O contrato passa a ser verificado no CI, e a superfície pública encolhe:** o `openapi.yaml` é a fonte de verdade e a DoD manda atualizá-lo antes do código — mas **nada verificava isso**, e a falta cobrou nas duas direções (rota prometida e ausente; sete `_status` **públicas e não documentadas**, duas respondendo `{"status": "stub"}`). Auditoria: **55 no contrato, 62 no app**. Os andaimes saíram (pentest F3.5 vem aí) e um teste compara os dois **caminho a caminho**, nomeando a rota que divergiu — comparar contagem deixaria duas trocas se cancelarem, e varrer `app.routes` devolveria **zero** rotas nesta versão do FastAPI, passando sempre **[novo]** |

| 116 | **A adesão passa a ser medida pelo áudio, e o encerramento deixa de regredir:** achado na **primeira execução da etapa (ii)**, com o app em aparelho real. O tempo efetivo vinha de um `Timer` da tela, e o Android **suspende a isolate do Dart** em segundo plano — tela bloqueada, que é o comportamento esperado numa sessão de 20 min. O áudio seguia no player nativo e o cronômetro parava: 20 minutos ouvidos chegaram como `effective_seconds: 0`, com ganho nulo. Em uso real a **adesão, que é desfecho primário, mediria zero o estudo inteiro** — e nenhum teste pegaria, porque `pump` faz o `Timer` disparar e o aparelho não. Agora o tempo é a **posição do player**, e a pausa é o relógio de parede menos o que soou. Segundo defeito, somado: `complete` reatribuía tudo a cada chamada, e ele é chamado **duas vezes de propósito** (o item de relaxamento vem depois da adesão) além dos reenvios da fila — um envio atrasado rebaixava sessão inteira em silêncio. Virou **monotônico**: envio com tempo menor é aceito mas não substitui, só complementa o relaxamento. O 409 que eu propus primeiro **quebraria** o complemento que o ADR-107 abriu de propósito **[novo]** |
