...
| B2 | Consentimento **específico e destacado** para dado sensível | 🟡 | O mecanismo registra a versão aceita; **conteúdo rascunhado** (`docs/tcle-rascunho.md`) na estrutura da Res. CNS 466/2012, com seção destacada de dado sensível (§12), retenção pós‑desistência explícita (§13, item D3) e recusa/desistência sem prejuízo (§11, risco **R‑09** do RIPD). Backend, app e resumo exibido já **sincronizados** na versão `0.1.0-rascunho` — o sufixo impede que um aceite de hoje seja lido depois como consentimento a termo aprovado; o resumo do app passou a citar evidência limitada, "não é tratamento", sorteio/cegamento, dado de saúde e **retenção pós‑desistência**. **Falta:** base legal (**A2**), preencher contatos/CEP/DPO, e a **aprovação do CEP** — inclusive sobre a via digital bastar e sobre exibir o texto integral no app (§N4 do rascunho). |
...
| G3 | Registro das operações de tratamento (Art. 37) | 🟡 | **Rascunho consolidado** (`docs/registro-operacoes-tratamento.md`): 8 operações (OP-01…08, com os desdobramentos OP-04b — rastreio de segurança — e OP-04c — descontinuação de protocolo) com finalidade, dados, base legal, operadores, transferência, retenção e segurança. **Falta** o controlador **adotar/manter** o registro formal e definir a base legal (A2). |
