...
| C8 | Trilha de auditoria **append‑only**, sem PII/braço | 🟡 | Guard na aplicação **implementado e testado** (barra UPDATE/DELETE antes do flush; `audit/service.py`, ADR‑056); ações sensíveis auditadas (consentimento, alocação, export, erase, desbloqueio, staff). **Falta** aplicar o `REVOKE UPDATE, DELETE` no Postgres via migração (hoje é só a intenção documentada) — defesa em profundidade no nível do banco. |
