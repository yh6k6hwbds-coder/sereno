...
def test_record_consent_ok_and_persists(api):
    ...
    r = client.post(CONSENT_URL, headers=hdr, json={"tcle_version": TCLE_CURRENT, "accepted": True})
    ...
    with TestSession() as s:
        ...
        assert rec.tcle_version == TCLE_CURRENT and rec.accepted is True
...
def test_missing_field_returns_422(api):
    client, TestSession = api
    _pid, hdr = _seed_participant_and_headers(TestSession)
    r = client.post(CONSENT_URL, headers=hdr, json={"tcle_version": TCLE_CURRENT})
...
def test_no_token_returns_401(api):
    client, _ = api
    r = client.post(CONSENT_URL, json={"tcle_version": TCLE_CURRENT, "accepted": True})
...
def test_declining_is_recorded(api):
    client, TestSession = api
    pid, hdr = _seed_participant_and_headers(TestSession)
    r = client.post(CONSENT_URL, headers=hdr, json={"tcle_version": TCLE_CURRENT, "accepted": False})
